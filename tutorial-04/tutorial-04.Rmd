...
```{r}
# Visualise respone variable
d %>% 
  ggplot() +
  geom_point(aes(x = x, y = y, colour = factor(y)),
             size = 5) +
  colorspace::scale_colour_discrete_qualitative() 

# options(digits = 3)
# d_aug %>% 
#   # Convert y to numeric variable
#   mutate(y = as.numeric(as.character(y))) %>% 
#   ggplot() +
#   geom_point(aes(x = x, y = .fitted, colour = factor(y)),
#              size = 8) +
